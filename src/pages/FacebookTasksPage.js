...
async function loadUserData() {
  ...
  try {
    ...
    if (session?.user?.id) {
      const { data: customer } = await CustomerService.getById(session.user.id);
      state.currentCustomer = customer || { id: session.user.id, facebook_name: session.user.email };
      state.walletInfo = await WalletService.getWalletInfo(state.currentCustomer.id);
    }
  } catch (err) {
